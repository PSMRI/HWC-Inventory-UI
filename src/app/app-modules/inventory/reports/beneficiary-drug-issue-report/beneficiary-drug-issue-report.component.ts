...
export class BeneficiaryDrugIssueReportComponent implements OnInit, DoCheck {
  ...
  searchReport() {
    const startDate: Date = new Date(
      this.beneficiaryDrugIssueForm.value.startDate,
    );
    const endDate: Date = new Date(this.beneficiaryDrugIssueForm.value.endDate);
    ...
    console.log(
      'Data form value...',
      JSON.stringify(this.beneficiaryDrugIssueForm.value),
    );
    const reqObjForBeneficiaryDrugIssueReport = {
      startDate: new Date(
    ...
    console.log(
      'Data form data',
      JSON.stringify(reqObjForBeneficiaryDrugIssueReport, null, 4),
    );
  ...
  exportToxlsx(criteria: any) {
    ...
    if (this.beneficiaryDrugIssueList.length > 0) {
      ...
      if (array.length !== 0) {
        ...
        this.beneficiaryDrugIssueList.forEach((row: { [x: string]: any }) => {
          const rowData: any[] = [];
          head.forEach((header) => {
            rowData.push(row[header]);
          });
      ...
      this.confirmationService.alert(
        this.currentLanguageSet.inventory.beneficiaryDrugIssueReportdownloaded,
        'success',
      );
    } else {
      this.confirmationService.alert(
