...
export class ExpiryReportComponent implements OnInit, DoCheck {
  ...
  get startDate() {
    return this.expiryForm.controls['startDate'].value;
  }

  get endDate() {
    return this.expiryForm.controls['endDate'].value;
  }
  ...
  searchReport() {
    const startDate: Date = new Date(this.expiryForm.value.startDate);
    const endDate: Date = new Date(this.expiryForm.value.endDate);
  ...
  exportToxlsx(criteria: any) {
    ...
    if (this.expiryList.length > 0) {
      ...
      if (array.length !== 0) {
        ...
        this.expiryList.forEach((row: { [x: string]: any }) => {
          const rowData: any[] = [];
          head.forEach((header) => {
            rowData.push(row[header]);
          });
      ...
      this.confirmationService.alert(
        this.currentLanguageSet.inventory.expiryReportdownloaded,
        'success',
      );
    } else {
      this.confirmationService.alert(
  ...
  trackFieldInteraction(fieldName: string) {
    this.trackingService.trackFieldInteraction(fieldName, 'Expiry Report');
  }
}
